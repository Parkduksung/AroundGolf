...
public class AgentInfo implements Parcelable {
	...
	public boolean getExplorerPermission() {
		if (checkExplorerUsable()) {
			if (checkExplorerOemType()) {
				if (checkExplorerSupportDevice()) {
                    if(VersionUtil.isSupportWindowsLoginAuthInFileExplorer(RuntimeData.getInstance().appProperty.getWebserverProductVersion())) {
                        return checkPermission(ComConstant.RVPOPTS_REXPLORER, ComConstant.RVPOPTS2_REXPLORER);
                    }else {
                        //서버 버전이 탐색기 윈도우인증 미지원인 경우, ASP는 차단, 그 외(솔루션납품)는 연다(기존 조건대로 처리)
//                        if (WebConnection.getInstance().isAspServer()) {
//                            RLog.i("FileExplorer not supports: Server version=" + RuntimeData.getInstance().appProperty.getWebserverProductVersion());
//                        }else {
//                            RLog.i("FileExplorer Windows auth not support, but CUSTOMIZED SERVER");
//                            return checkPermission(ComConstant.RVPOPTS_REXPLORER, ComConstant.RVPOPTS2_REXPLORER);
//                        }
                    }
				}else {
					RLog.i("FileExplorer not supports: Device not supports");
				}
			}else {
				RLog.i("FileExplorer not supports: OEMTYPE");
			}
		}else {
			RLog.i("FileExplorer not supports: Server configured");
		}
		return false;
	}
	...
	private boolean checkExplorerSupportDevice() {
		...
		try {
			...
			RLog.e("checkExplorerSupportDevice", e.getLocalizedMessage());
			return false;
		}
		switch (extendConst) {
